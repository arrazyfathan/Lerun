...
import com.arrazyfathan.lerun.feature.run.R
import com.arrazyfathan.lerun.feature.run.databinding.FragmentRunBinding
...
class RunFragment : Fragment(R.layout.fragment_run), EasyPermissions.PermissionCallbacks {
    ...
    private fun setupView() {
        binding.chart.xAxis.apply {
            ...
            setDrawLabels(false)
            axisLineColor = ContextCompat.getColor(requireContext(), UiR.color.dark_accent_text)
            textColor = ContextCompat.getColor(requireContext(), UiR.color.dark_accent_text)
        ...
        viewModel.runsSortedByDateAsc.observe(viewLifecycleOwner) { runs ->
            ...
            runs?.let { data ->
                ...
                val barDataSet = LineDataSet(allAvgSpeed, "Average Speed Overtime").apply {
                    ...
                    color = ContextCompat.getColor(requireContext(), UiR.color.ijo)
                    mode = LineDataSet.Mode.CUBIC_BEZIER
                    setCircleColor(ContextCompat.getColor(requireContext(), UiR.color.ijo))
                    setDrawCircleHole(false)
                    setDrawFilled(true)
                    fillDrawable = ContextCompat.getDrawable(requireContext(), UiR.drawable.gradient_green)
                }
    ...
    private fun getToday() {
        ...
        when (calendar.get(Calendar.HOUR_OF_DAY)) {
            in 0..11 -> {
                binding.tvDateToday.text = getString(UiR.string.good_morning)
            }

            in 12..15 -> {
                binding.tvDateToday.text = getString(UiR.string.good_afternoon)
            }

            in 16..20 -> {
                binding.tvDateToday.text = getString(UiR.string.good_evening)
            }

            in 21..23 -> {
                binding.tvDateToday.text = getString(UiR.string.good_night)
            }
        }
    }
