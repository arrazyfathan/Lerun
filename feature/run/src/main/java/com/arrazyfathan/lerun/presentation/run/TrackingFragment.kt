...
import com.arrazyfathan.lerun.feature.run.databinding.FragmentTrackingBinding
...
class TrackingFragment :
    ...
    private fun showDialogFinish() {
        val dialog = BottomSheetDialog(requireContext(), UiR.style.BottomSheetDialogTheme)
    ...
    private fun stopRun() {
        binding.tvTimer.text = getString(UiR.string.default_timer)
        sendCommandToService(ACTION_STOP_SERVICE)
        navigator.closeTrackingToRun()
    }

    private fun updateTracking(isTracking: Boolean) {
        this.isTracking = isTracking
        if (!isTracking && currentTimeMillis > 0L) {
            binding.textStart?.text = getString(UiR.string.start_label)
            binding.icBtnStart?.setImageDrawable(
                ContextCompat.getDrawable(
                    requireContext(),
                    UiR.drawable.ic_play,
                ),
            )
            binding.btnFinishRun.visibility = View.VISIBLE
        } else if (isTracking) {
            binding.textStart?.text = getString(UiR.string.stop_label)
            binding.icBtnStart?.setImageDrawable(
                ContextCompat.getDrawable(
                    requireContext(),
                    UiR.drawable.ic_stop,
                ),
            )
            menu?.getItem(0)?.isVisible = true
